...
Install [`wordpress/wb-app-pages.php`](wordpress/wb-app-pages.php) into `wp-content/mu-plugins/` first. Content Mask keeps its configuration in post meta, and the stock REST API silently drops meta that isn't registered with `show_in_rest` — posting to `/wp/v2/pages` would return 201 and create an unmasked, blank page. The companion plugin exposes one route (`POST /wp-json/wb/v1/app-page`) and owns the meta keys, which live in a single constant (`WB_CONTENT_MASK_META`) at the top of that file. Verify them against your install with `wp post meta list <page-id>` on a page you masked by hand.
